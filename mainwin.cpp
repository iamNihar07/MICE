...
Mainwin::Mainwin() {
    ...
    createmenu->append(*menuitem_server);
     ...
     //         M A N A G E R
    // (Owner) “Create a New Manager” Append Manager to the Create menu
    ...
    //     S H O W    O R D E R
    // Show order menu and add to the menu bar
    Gtk::MenuItem *menuitem_show_order = Gtk::manage(new Gtk::MenuItem("_Show Order", true));
    menubar->append(*menuitem_show_order);
    Gtk::Menu *ordermenu = Gtk::manage(new Gtk::Menu());
    menuitem_show_order->set_submenu(*ordermenu);

    //         BY ORDER I D
    // (All) Shows Order by order ID
    menuitem_byID = Gtk::manage(new Gtk::MenuItem("_By Order ID", true));
    menuitem_byID->signal_activate().connect(sigc::mem_fun(*this, &Mainwin::on_byID_click));
    ordermenu->append(*menuitem_byID);
